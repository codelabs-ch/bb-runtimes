...
procedure Initialize_CPU is
   ...
   ---------------------------
   -- Set_Clock_Frequencies --
   ---------------------------

   procedure Set_Clock_Frequencies;

   procedure Set_Clock_Frequencies is
   begin
      APIC_Frequency_In_kHz := Musinfo.Instance.TSC_Khz;
      TSC_Frequency_In_kHz  := APIC_Frequency_In_kHz;
   end  Set_Clock_Frequencies;
...
begin
   ...
   begin
      --  Check for XSAVE processor feature and TSC
   ...
   if not Musinfo.Instance.Is_Valid then
      raise Program_Error with "Muen sinfo not valid";
   end if;
